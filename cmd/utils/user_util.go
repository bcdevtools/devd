...
func GetPseudoDefaultGroupForUser(username string) string {
	if IsLinux() {
		return username
	} else if IsDarwin() {
		return "staff"
...
func TryExtractUserNameFromHomeDir(homeDir string) (username string, success bool) {
	defer func() {
		if !success {
			username = ""
		}
	...
	if !libutils.IsBlank(homeDir) {
		homeDir = strings.TrimSpace(homeDir)

		if homeDir == "/root" || strings.HasPrefix(homeDir, "/root/") {
			username = "root"
			success = true
		} else {
			rootHomeDir := GetUserGeneralRootDirLocation()
			if strings.HasPrefix(homeDir, rootHomeDir+"/") {
				username = strings.TrimPrefix(homeDir, rootHomeDir)
